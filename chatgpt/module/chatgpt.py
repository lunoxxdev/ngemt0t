...
@ren.on_message(filters.command("tanya") & filters.private | filters.group)
async def chatgpt(c: Client, m: Message):
    ...
    if not randydev:
       await m.reply(f"Kalo sehabis <code>/{m.command[0]} [question]</code> kasih deskripsi cok asui 😒.")
       return
    headers = {
